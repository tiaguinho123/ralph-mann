...
export default function ContactPage() {
  ...
  return (
    <div className="pt-20">
      {/* Banner */}
      <div className="relative h-48 overflow-hidden" style={{ backgroundColor: '#1c2e6e' }}>
      ...
      <div className="bg-slate-50 border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="text-sm text-slate-500">
            <Link to="/" className="hover:text-[#c0392b] transition-colors">Home</Link>
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <div className="space-y-7 mb-10">
                <div className="flex items-start gap-4">
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: '#c0392b1A' }}>
                    <Phone className="w-5 h-5" style={{ color: '#c0392b' }} aria-hidden="true" />
                  </div>
                  <div>
                    <p className="font-bold text-slate-900 mb-1">Phone</p>
                    <a href={`tel:${phone}`} className="text-2xl font-extrabold transition-colors" style={{ color: '#c0392b' }}>{phoneFormatted}</a>
                ...
                <div className="flex items-start gap-4">
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: '#c0392b1A' }}>
                    <Mail className="w-5 h-5" style={{ color: '#c0392b' }} aria-hidden="true" />
                  </div>
                  <div>
                    <p className="font-bold text-slate-900 mb-1">Email</p>
                    <a href={`mailto:${email}`} className="text-slate-700 hover:text-[#c0392b] transition-colors">{email}</a>
                  </div>
                </div>

                <div className="flex items-start gap-4">
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: '#c0392b1A' }}>
                    <MapPin className="w-5 h-5" style={{ color: '#c0392b' }} aria-hidden="true" />
                  </div>
                  <div>
                ...
                <div className="flex items-start gap-4">
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: '#c0392b1A' }}>
                    <Clock className="w-5 h-5" style={{ color: '#c0392b' }} aria-hidden="true" />
                  </div>
                  <div>
                    ...
                    {hours.emergency && <p className="font-bold mt-1" style={{ color: '#c0392b' }}>Emergency: 24 Hours / 7 Days</p>}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-50 rounded-3xl p-8 border border-slate-100">
                {submitted ? (
                  <div className="text-center py-12">
                    <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6" style={{ backgroundColor: '#c0392b' }}>
                      <Send className="w-8 h-8 text-white" aria-hidden="true" />
                    </div>
                    <h3 className="text-2xl font-bold text-slate-900 mb-3">Message Sent!</h3>
                    <p className="text-slate-600">We'll be in touch shortly. For urgent needs, call <a href={`tel:${phone}`} className="font-bold" style={{ color: '#c0392b' }}>{phoneFormatted}</a>.</p>
                  </div>
                ) : (
                  <>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5" aria-label="Contact form">
                      <div className="grid sm:grid-cols-2 gap-5">
                        <div>
                          ...
                          <input id="contact-first" type="text" required autoComplete="given-name" placeholder="John"
                            className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-red-200 focus:border-[#c0392b] bg-white transition" />
                        </div>
                        <div>
                      ...
                      <div>
                        ...
                        <textarea id="contact-message" rows={4} placeholder="Describe your HVAC issue or ask a question..."
                          className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-red-200 focus:border-[#c0392b] bg-white transition resize-none" />
                      </div>
                      <button type="submit"
